...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negaatisen_summan_lataaminen_ei_muuta_kortin_saldoa(self):
        self.kortti.lataa_rahaa(-2)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_kortilla_pystyy_ostamaan_edullisen_lounaan_jos_saldo_on_edullisen_lounaan_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_kortilla_pystyy_ostamaan_maukkaan_lounaan_jos_saldo_on_maukkaan_lounaan_verran(self):
